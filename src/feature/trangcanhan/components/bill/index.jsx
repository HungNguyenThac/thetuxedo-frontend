...
function BillOFUser() {
  ...
  const postBillForUserModol = async () => {
    const cloneBill = bills;
    const historyBill = cloneBill.splice(index, 1);
    const cloneStories = stories;
    cloneStories.unshift(historyBill[0]);

    try {
      let responseBill = await userApi.putAddCurrentBill(cloneBill);
      let responseHistory = await userApi.putAddHistoryBill(cloneStories);
      const { status, user } = responseBill;
      ...
      if (responseHistory.status === 200) {
        dispatch(dispatchHistory(responseHistory.user.history));
      }

      if (responseHistory.status === 200 && status === 200) {
                      ...
                      <div className="form-user-profile">
                        ...
                        <div className="note-for-user">
                          ...
                          <ul className="note-for-user_ul">
                            ...
                            <li className="note-for-user_content">
                              * Sản phẩm được bảo hành trọn đời theo chính sách
                              The TUXEDO
                            </li>
                            <li className="note-for-user_content">
              ...
              <footer className="footer-cartPage">
                ...
                <div className="footer-cartPage_bottom">
                  <button className="button-add-item-cart" disabled>
                    ...
                    <span className="button-add-item-cart_sub">Khiếu nại</span>
                  </button>
                  <button
